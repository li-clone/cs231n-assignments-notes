...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        x_hat = x_centered / np.sqrt(sample_var + eps)
...
def batchnorm_backward(dout, cache):
    ...
    x_centered = x - sample_mean
    dvar = np.sum(
        dx_hat * x_centered * (-0.5) * (sample_var + eps) ** (-1.5),
        axis=0,
    )

    dx_centered_direct = dx_hat / np.sqrt(sample_var + eps)
    dx_centered_var = dvar * 2 * x_centered / N
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    var = np.mean(x_centered ** 2, axis=(2, 3, 4), keepdims=True)
    inv_std = 1.0 / np.sqrt(var + eps)
    x_hat_group = x_centered * inv_std
    x_hat = x_hat_group.reshape(N, C, H, W)

    out = gamma * x_hat + beta
    cache = (G, x_hat, gamma, inv_std)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dx_group = (inv_std / group_size) * (
        ...
        - x_hat_group
        * np.sum(dx_hat_group * x_hat_group, axis=(2, 3, 4), keepdims=True)
    )
