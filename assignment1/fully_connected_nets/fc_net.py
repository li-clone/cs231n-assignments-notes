...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            out, caches[i] = affine_relu_forward(
                out,
                self.params[f"W{i}"],
                self.params[f"b{i}"],
            )

        # The output layer is affine only; its output is the class scores.
        ...
        # Add L2 regularization for every weight matrix. Biases are not
        # regularized. The factor 0.5 makes the gradient simply reg * W.
        for i in range(1, self.num_layers + 1):
            W = self.params[f"W{i}"]
        ...
        grads[f"W{self.num_layers}"] = dW + self.reg * self.params[
            f"W{self.num_layers}"
        ]
        grads[f"b{self.num_layers}"] = db

        # Backpropagate through hidden affine-ReLU layers in reverse order.
