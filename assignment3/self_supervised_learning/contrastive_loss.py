...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        for anchor_idx, positive_idx in ((k, k + N), (k + N, k)):
            numerator = torch.exp(
                sim(out[anchor_idx], out[positive_idx]) / tau
            )

            denominator = 0
            for other_idx in range(2 * N):
                if other_idx != anchor_idx:
                    denominator = denominator + torch.exp(
                        sim(out[anchor_idx], out[other_idx]) / tau
                    )
...
def sim_positive_pairs(out_left, out_right):
    ...
    left_norms = torch.linalg.norm(out_left, dim=1, keepdim=True)
    right_norms = torch.linalg.norm(out_right, dim=1, keepdim=True)
    pos_pairs = dot_products / (left_norms * right_norms)
