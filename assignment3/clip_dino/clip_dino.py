...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    processed_images = [
        clip_preprocess(Image.fromarray(image)).unsqueeze(0)
        for image in images
    ]
    image_tensor = torch.cat(processed_images, dim=0).to(device)
    image_features = clip_model.encode_image(image_tensor)

    similarity = get_similarity_no_loop(text_features, image_features)
    pred_indices = similarity.argmax(dim=0)
    pred_classes = [class_texts[index] for index in pred_indices.tolist()]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
