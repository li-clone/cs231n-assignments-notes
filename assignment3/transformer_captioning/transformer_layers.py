...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        q = self.query(query).reshape(N, S, self.n_head, self.head_dim)
        k = self.key(key).reshape(N, T, self.n_head, self.head_dim)
        v = self.value(value).reshape(N, T, self.n_head, self.head_dim)
        ...
        attention = self.attn_drop(F.softmax(scores, dim=-1))
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        tgt = self.norm_cross(tgt + shortcut)

        shortcut = tgt
        tgt = self.ffn(tgt)
        tgt = self.dropout_ffn(tgt)
        tgt = self.norm_ffn(tgt + shortcut)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
